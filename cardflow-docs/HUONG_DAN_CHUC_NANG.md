...
## 🏛️ 5. Kiến Trúc Fullstack Monorepo (Next.js + Go Service)

Mã nguồn dự án đã được tích hợp thành **Fullstack Monorepo** thống nhất:

```text
Card-app/ (Fullstack Monorepo)
├── apps/
│   ├── web/               # Next.js 16 (Giao diện Web Frontend)
│   ├── mobile/            # Expo (App Mobile)
│   └── backend/           # Go Service (Echo v4 + PostgreSQL + Redis + go-kit)
├── shared/                # Dùng chung TypeScript Types, DTOs, Tokens, Env config
├── package.json           # Lệnh điều phối toàn dự án
├── pnpm-workspace.yaml    # Workspace pnpm quản lý 4 gói
└── turbo.json             # Turborepo task pipeline
```

### 🚀 Lệnh Khởi Chạy Nhanh Cho Developer:

1. **Khởi động Database (PostgreSQL + Redis)**:
   ```bash
   pnpm db:up
   # Khởi chạy PostgreSQL trên port 5432 và Redis trên port 6379 qua Docker
   ```

2. **Chạy Backend Go (Echo API Server - Port 8080)**:
   ```bash
   pnpm backend
   # Chạy HTTP Server tại http://localhost:8080
   ```

3. **Chạy Frontend Web (Next.js 16 - Port 3000)**:
   ```bash
   pnpm --filter @cardflow-app/web dev
   # Truy cập Dashboard tại http://localhost:3000/dashboard
   ```

4. **Chạy đồng thời toàn bộ hệ thống**:
   ```bash
   pnpm dev
   ```

5. **Dừng Database khi hoàn tất**:
   ```bash
   pnpm db:down
   ```
